...
const Printing = () => {
  ...
  const formatResponseValue = responseObj => {
    if (!responseObj) return { answer: '', reason: '', serialNol' }
    return {
  ...
  const handleSubmit = async () => {
    try {
      const dataToSend = {
        ...
        sideLay: formatResponseValue(responses['sideLay']), // remove space
        frontLay: formatResponseValue(responses['frontLay']), // remove space
        registration: formatResponseValue(responses['registration']),
        scumming: formatResponseValue(responses['scumming']),
        setOff: formatResponseValue(responses['setOff']),
  ...
  // console.log('Saving', { header, serialNo, value })
                                      ...
                                      editingCell === cellKey ? (
                                        <Select
                                          ...
                                          onChange={e => {
                                            ...
                                            const [rowIndex, colIndex] = cellKey.split('_').map(Number)
                                            const serialNo = rows[rowIndex]?.serial
                                            const header = questions[colIndex]
