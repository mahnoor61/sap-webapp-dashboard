...
const Food = () => {
  ...
  const getData = async () => {
    try {
      ...
      console.log('error in get data through id in food', error.response)
    }
  }
  ...
  useEffect(() => {
    const savedResponses = localStorage.getItem('food_responses')
    if (savedResponses) {
  ...
  useEffect(() => {
    if (responses && Object.keys(responses).length > 0) {
      localStorage.setItem('food_responses', JSON.stringify(responses))
    }
  }, [responses])
  ...
  const handleSubmit = async () => {
    ...
    const allFilled = requiredFields.every(field => responses[field]?.answer)
    ...
    try {
      const dataToSend = {
        ...
        shift: shift,
        date: formattedDate,
        printingSpots: formatResponseValue(responses['PrintingSpots']),
        ccWrongCutting: formatResponseValue(responses['CcWrongCutting']),
        embossOut: formatResponseValue(responses['EmbossOut']),
        laminationWrinkle: formatResponseValue(responses['LaminationWrinkle']),
      ...
      localStorage.removeItem('food_responses')
    } catch (err) {
  ...
  if (jobId) {
    const getAllQCList = async () => {
      try {
        const response = await axios.post(
          `${BASE_URL}/api/ap/qc/get/qc-data/food`,
          {
  ...
  function printReceipt() {
    const printContents = document.getElementById('receipt-food')?.innerHTML
      ...
      <style>
        @media print {
          ...
          .print-table-container {
            width: 100%;
            min-width: 1000px; /* ensure it fits landscape layout */
          }
          table {
            width: 100%;
          ...
          {paginatedUsers
            .flatMap((data, index) => {
              ...
              const fields = {
                ...
                ccWrongCutting: 'CcWrongCutting',
                embossOut: 'EmbossOut',
                laminationWrinkle: 'LaminationWrinkle',
                bubble: 'Bubble',
                files: 'Files',
              ...
              for (const key in fields) {
                if (form[key]?.answer === 'Not Okay') {
                  failedFields.push({
                    number: page * rowsPerPage + index + 1, // ✅ fixed global serial
                    field: fields[key],
                    reason: form[key]?.reason || 'No reason provided'
                  })
                }
              }
                ...
                >
                  <thead style={{ backgroundColor: 'skyblue', width: '100%' }}>
                    <tr>
                      ...
                      <th style={{ padding: '6px', border: '1px solid #ccc', fontWeight: 'bold' , width:'10'}}>CcWrongCutting</th>
