    ...
    <>
      ...
      >
        ...
        <DialogContent>
          ...
          {/* const makeTimeUsers = users.filter(user => user.makeTimeStatus); const normalUsers = users.filter(user =>
          !user.makeTimeStatus); */}
          ...
          <Table size='small' sx={{ border: '1px solid #ccc', mt: 4, mb: 4 }}>
            <TableHead sx={{ bgcolor: 'skyblue' }}>
              <TableRow>
                ...
                {users.map(user => (
                <TableCell key={user._id} sx={{ fontWeight: 'bold', whiteSpace: 'nowrap', textAlign: 'center' }}>
  <Box>
    <div>{new Date(user.time).toLocaleTimeString()}</div>
    <Box
      sx={{
        width: '100%',
        height: '1px',
