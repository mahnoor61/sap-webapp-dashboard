...
const AllOperatorData = () => {
  ...
  const roleBasedProductionList = auth?.user?.role === 'operator' ? FilterHelper(allowedOrders, searchQuery, ['productionOrderNo', 'status']) : FilterHelper(prodctionLists, searchQuery, ['productionOrderNo', 'status', 'machine'])
