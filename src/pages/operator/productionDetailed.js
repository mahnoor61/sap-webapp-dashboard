...
import {
  Grid,
  Card, TextField,
  CardContent, Box, Button, FormControl, FormLabel, RadioGroup, FormControlLabel, Radio, FormHelperText,
...
// function for issue for machine
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        const response = await axios.post(`${BASE_URL}/api/ap/operator/update/recieved-by-operator/${productionOrderDetail._id}`, {
  ...
  const issueFormik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        const response = await axios.post(`${BASE_URL}/api/ap/operator/add/issue-for-machine`, {
  ...
  const completeFormik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        const response = await axios.post(`${BASE_URL}/api/ap/operator/add/completed-quantity`, {
          id: productionOrderDetail._id,
  ...
  const wastedFormik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        const response = await axios.post(`${BASE_URL}/api/ap/operator/add/wasted-quantity`, {
          id: productionOrderDetail._id,
  ...
  const getRemainingQty = async () => {
    try {
      const response = await axios.get(`${BASE_URL}/api/ap/operator/get/remaining-quantity/${productionOrderDetail._id}`, {
        headers: {
          "x-access-token": token
        }
      });
  ...
  const getTotalCompletedQuantityOfPreviousRoute = async () => {
    try {
      const response = await axios.post(`${BASE_URL}/api/ap/operator/get/completed-quantity/previous-route`, {
        id: productionOrderDetail._id,
  ...
  const handleStartProduction = async () => {
    try {
      ...
      const res = await axios.post(`${BASE_URL}/api/ap/operator/production/order/update/make-time`, {
        id: productionOrderDetail._id,
  ...
  const updateStatus = async (status) => {
    try {
      const response = await axios.post(`${BASE_URL}/api/ap/operator/update/status`, {
        id: productionOrderDetail._id,
        status: status
      }, {
        headers: {
  ...
  const handlePauseProduction = async () => {
    try {
      ...
      const PauseReason = pauseFormik.values.pauseReason === 'Other'
      ...
      await axios.post(`${BASE_URL}/api/ap/operator/production/order/update/production-time`, {
        id: productionOrderDetail._id,
  ...
  const handleResumeProduction = async () => {
    try {
      const res = await axios.get(
        `${BASE_URL}/api/ap/operator/get/prodTime/${productionOrderDetail._id}`,
        {
          headers: {"x-access-token": token},
        }
      );
  ...
  const handleDownTimeProduction = async () => {
    try {
      ...
      const DownReason = downFormik.values.downReason === 'Other'
        ? downFormik.values.customDownReason
        : downFormik.values.downReason;
      ...
      // Save production timer to API
      await axios.post(`${BASE_URL}/api/ap/operator/update/downtime`, {
        id: productionOrderDetail._id,
        startProductionTime: formattedProductionTime,
  ...
  //   try {
  //     const response = await axios.get(`${BASE_URL}/api/ap/operator/get/last/route/${order}`, {
  //       headers: {
  //         "x-access-token": token
  //       }
  //     });
  //     const data = response.data.data.isCurrentRouteIsLastRoute;
  //     setIsLastRoute(data);
  //   } catch (error) {
  //     console.error('error in get last route api', error);
  //   }
  // }
  //
  // useEffect(() => {
                    ...
                    <TableContainer sx={{width: '100%', overflowX: 'auto'}}>
                      <Table size="small">
                        <TableBody>
                          ...
                          <TableRow>
                            <TableCell sx={{width: '50%'}}><strong>Previous Route Completed
                              Quantty:</strong></TableCell>
                ...
                <DialogContent>
                  <DialogContentText id="alert-dialog-description">
                    <TextField fullWidth id="outlined-basic" variant="filled"
                               ...
                               type="number"
                               error={Boolean(completeFormik.touched.completedQuantity && completeFormik.errors.completedQuantity)}
