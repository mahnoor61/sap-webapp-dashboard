...
const Pasting = () => {
  ...
  const getAllJobsOfSelectedMachine = async machineId => {
    ...
    try {
      const response = await axios.get(`${BASE_URL}/api/ap/qc/get/machine-data/pasting/${machineId}`, {
        headers: {
      ...
      console.log('error in get all jobs of selected machine in pasting', error)
      toast.error(error)
      setLoadingComplete(false)
    }
  }
    ...
    onSubmit: async values => {
      try {
        ...
        const response = await axios.post(
          `${BASE_URL}/api/ap/qc/add-quantity/pasting`,
          {
        ...
        router.push(`/quality-control/pasting?id=${response.data.data._id}`)
        handleClose()
        formik.resetForm()
      } catch (error) {
  ...
  const getAllLaminatingMachines = async () => {
    try {
      const response = await axios.get(`${BASE_URL}/api/ap/qc/get/pasting-machine`, {
        headers: {
          'x-access-token': token
        }
      })
      ...
      console.log('error in get all pasting machines', error)
      toast.error(error)
    }
  }
  ...
  return (
    <>
      <Stack spacing={1} sx={{ width: '100%' }}>
        <Chip sx={{ textAlign: 'center', mb: 2, width: '100%', mt: 5 }} label='Pasting' />
      </Stack>
              ...
              paginatedJob.map(data => (
                ...
                >
                  ...
                  <TableCell component='th' scope='row'>
                    <NextLink
                      href={{
                        pathname: '/quality-control/pasting',
