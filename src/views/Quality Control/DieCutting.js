...
const DieCutting = () => {
  ...
  const getAllJobsOfSelectedMachine = async machineId => {
    ...
    try {
      ...
      console.log('error in get all jobs of selected machine in die', error)
      toast.error(error)
      setLoadingComplete(false)
    }
  }
    ...
    onSubmit: async values => {
      try {
        ...
        const response = await axios.post(
          `${BASE_URL}/api/ap/qc/add-quantity/die`,
          {
        ...
        router.push(`/quality-control/die?id=${response.data.data._id}`)
        handleClose()
        formik.resetForm()
      } catch (error) {
  ...
  const getAllDieMachines = async () => {
    try {
      const response = await axios.get(`${BASE_URL}/api/ap/qc/get/die-cutting-machine`, {
        headers: {
          'x-access-token': token
        }
      })
      ...
      console.log('error in get all die cutting machines', error)
      toast.error(error)
    }
  }
  ...
  return (
    <>
      <Stack spacing={1} sx={{ width: '100%' }}>
        <Chip sx={{ textAlign: 'center', mb: 2, width: '100%', mt: 5 }} label='Die Cutting' />
      </Stack>
