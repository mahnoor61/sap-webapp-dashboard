...
const Laminating = () => {
  ...
  const getAllJobsOfSelectedMachine = async machineId => {
    ...
    try {
      const response = await axios.get(`${BASE_URL}/api/ap/qc/get/machine-data/laminating/${machineId}`, {
        headers: {
      ...
      console.log('error in get all jobs of selected machine in laminating', error)
      toast.error(error)
      setLoadingComplete(false)
    }
  }
    ...
    onSubmit: async values => {
      try {
        ...
        const response = await axios.post(
          `${BASE_URL}/api/ap/qc/add-quantity/laminating`,
          {
        ...
        router.push(`/quality-control/laminating?id=${response.data.data._id}`)
        handleClose()
        formik.resetForm()
      } catch (error) {
  ...
  const getAllLaminatingMachines = async () => {
    try {
      const response = await axios.get(`${BASE_URL}/api/ap/qc/get/laminating-machine`, {
        headers: {
          'x-access-token': token
        }
      })
      ...
      console.log('error in get all laminating machines', error)
      toast.error(error)
    }
  }
  ...
  return (
    <>
      <Stack spacing={1} sx={{ width: '100%' }}>
        <Chip sx={{ textAlign: 'center', mb: 2, width: '100%', mt: 5 }} label='Laminating' />
      </Stack>
              ...
              paginatedJob.map(data => (
                ...
                >
                  ...
                  <TableCell component='th' scope='row'>
                    <NextLink
                      href={{
                        pathname: '/quality-control/laminating',
