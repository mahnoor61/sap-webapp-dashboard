...
const AddRoute = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: yup.object({
      routeCode: yup.string().required('Route is required'),
    }),
    onSubmit: async (values) => {

      try {

        const response = await axios.post(`${BASE_URL}/api/ap/admin/create/route`, {
        ...
        <form onSubmit={formik.handleSubmit}>
          <Grid container spacing={1} sx={{px: 20}}>
            <Stack spacing={1} sx={{ width: '100%'}}>
              <Chip sx={{textAlign: 'center', mb: 10, width: '100%'}} label="Add Routes " />
            ...
            <TextField
              ...
              helperText={formik.touched.routeCode && formik.errors.routeCode}
